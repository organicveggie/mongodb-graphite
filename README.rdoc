...
= Usage
 ...
 Metric Path Options:
     -h, --host-trim VALUE            Value to trim from the end of the hostname.
...
By default, mongodb-graphite takes the hostname, reverses it and appends the name of the metric from MongoDB. For example, the the default metric path for <em>opcounters.command</em> from <em>mongo1.example.com</com> looks like the following:
